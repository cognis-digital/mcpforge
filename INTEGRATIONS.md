...
**mcpforge** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `mcpforge` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
mcpforge ... --format json | cognis-connect emit --to sigma   # Sigma rules
mcpforge ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
mcpforge ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `mcpforge` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="mcpforge") for rec in records]   # records = your JSON output
